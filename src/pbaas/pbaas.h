...
template <typename TOBJ>
CTxOut MakeCCMofNVout(uint8_t evalcode, CAmount nValue, const std::vector<CTxDestination> &dests, int M, const TOBJ &obj)
{
    CTxOut vout;
    ...
    std::vector<std::vector<unsigned char>> vvch({::AsVector((const TOBJ)obj)});
    COptCCParams vParams = COptCCParams(COptCCParams::VERSION_V3, evalcode, M, (uint8_t)(dests.size()), dests, vvch);

    for (auto &dest : dests)
    {
        if (!GetDestinationBytes(dest).size())
        {
            printf("Invalid destination %s\n", EncodeDestination(dest).c_str());
        }
    }
...
{
    COptCCParams masterParams = COptCCParams(COptCCParams::VERSION_V3, 0, 1, 1, indexDest ? std::vector<CTxDestination>({*indexDest}) : std::vector<CTxDestination>(), std::vector<std::vector<unsigned char>>());
...
{
    if (M > 2) M = 2;
    COptCCParams masterParams = COptCCParams(COptCCParams::VERSION_V3, 0, M, 2, indexDest ? std::vector<CTxDestination>({*indexDest}) : std::vector<CTxDestination>(), std::vector<std::vector<unsigned char>>());
    CScript scriptRet = CScript() << masterParams.AsVector() << OP_CHECKCRYPTOCONDITION;
...
{
    if (M > 3) M = 3;
    COptCCParams masterParams = COptCCParams(COptCCParams::VERSION_V3, 0, M, 3, indexDest ? std::vector<CTxDestination>({*indexDest}) : std::vector<CTxDestination>(), std::vector<std::vector<unsigned char>>());
    CScript scriptRet = CScript() << masterParams.AsVector() << OP_CHECKCRYPTOCONDITION;
...
{
    if (M > 4) M = 4;
    COptCCParams masterParams = COptCCParams(COptCCParams::VERSION_V3, 0, M, 4, indexDest ? std::vector<CTxDestination>({*indexDest}) : std::vector<CTxDestination>(), std::vector<std::vector<unsigned char>>());
    CScript scriptRet = CScript() << masterParams.AsVector() << OP_CHECKCRYPTOCONDITION;
