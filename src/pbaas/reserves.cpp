...
CCurrencyValueMap::CCurrencyValueMap(const UniValue &uni)
{
    ...
    if (uni.isArray())
    {
        auto keys = uni.getKeys();
        auto values = uni.getValues();
        for (int i = 0; i < keys.size(); i++)
        {
            ...
            try
            {
                valueMap[currencyID] = AmountFromValue(values[i]);
            }
...
std::vector<CAmount> CCurrencyValueMap::AsCurrencyVector(const CCurrencyStateNew &cState) const
{
    ...
    for (auto &oneInOut : valueMap)
    {
        retVal[curMap[oneInOut.first]] = oneInOut.second;
    }
    return retVal;
}
        ...
        nativeConversionFees(0)
{
    ...
    if (IsReserve())
    {
        ...
        {
            if (!(cci.importValue == ccx.totalAmounts + ccx.totalFees))
            {
    ...
    for (int i = 0; i < exportObjects.size(); i++)
    {
        if (exportObjects[i]->objectType == CHAINOBJ_RESERVETRANSFER)
        {
            ...
            if (curTransfer.IsValid())
            {
                ...
                {
                    if (curTransfer.currencyID == currencyDest.systemID)
                    {
                        nativeIn += (curTransfer.nValue + curTransfer.nFees);
                    }
                    else
                    {
                        AddReserveInput(curTransfer.destCurrencyID, curTransfer.nValue + curTransfer.nFees);
                    }
                }
                ...
                if (curTransfer.flags & curTransfer.PRECONVERT)
                {
                    ...
                    if (!(curTransfer.flags & curTransfer.FEE_OUTPUT))
                    {
                        ...
                        preConversionFee += curTransfer.nFees;
                        ...
                        if (!currencyDest.IsToken() && !(currencyDest.ChainOptions() & currencyDest.OPTION_FEESASRESERVE))
                        {
                            if (curTransfer.destCurrencyID == systemDestID)
                            {
                                ...
                                newCurrencyConverted += newNativeFees;
                                preConversionFee = 0;
                            }
...
CMutableTransaction &CReserveTransactionDescriptor::AddConversionInOuts(CMutableTransaction &conversionTx, std::vector<CInputDescriptor> &conversionInputs, const CCurrencyValueMap &_exchangeRates, const CCurrencyStateNew *pCurrencyState) const
{
    ...
    {
        if (pCurrencyState && currencyState.IsReserve())
        {
